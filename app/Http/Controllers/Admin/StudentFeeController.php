...
class StudentFeeController extends Controller
{
    ...
    {
        $data['years'] = Academic::all();
        $data['classes'] = Cohort::all();
        $data['fee_categories'] = FeeCategory::all();
        return view('dashboard.account.student.fee.add', $data);
    }
    ...
    public function StudentFeeGetStudent(Request $request)
    {
        ...
        $data = AssignStudent::with(['discount'])->where('year_id', $year_id)->where('cohort_id', $cohort_id)->get();
        ...
        foreach ($data as $key => $std) {
            $registrationfee = FeeCategoryAmount::where('fee_category_id', $fee_category_id)->where('cohort_id', $std->cohort_id)->first();

            $accountstudentfees = StudentFee::where('student_id', $std->student_id)->where('year_id', $std->year_id)->where('cohort_id', $std->cohort_id)->where('fee_category_id', $fee_category_id)->where('date', $date)->first();
            ...
            $html[$key]['tdsource']  .= '<td>' . $std['student']['fname'] . '<input type="hidden" name="cohort_id" value= " ' . $std->cohort_id . ' " >' . '</td>';
